...
function runMonteCarlo(teamScores, playerData, cutLine, simCount = 10000) {
  ...
  // Royal Birkdale (like Shinnecock) plays to a par of 70
  const PAR = 70;
  for (let sim = 0; sim < simCount; sim++) {
    const simTeamTotals = {};
    for (const [teamName, golfers] of Object.entries(TEAMS)) {
      const golferSims = golfers.map(g => {
        const p = playerData[normalizeName(g)];
        if (!p || p.notStarted) {
          // Not started — simulate full 72 holes around even (Open setups play hard, esp. in wind)
...
function simulateRemainingHoles(currentScore, holesLeft, isMajorSetup) {
  if (holesLeft <= 0) return 0;
  // Major championship distribution: slightly harder than average tour setup
  // Mean per hole: slightly over par (+0.05 to +0.08 per hole), std dev ~0.9
  let total = 0;
  for (let i = 0; i < holesLeft; i++) {
    ...
    const holeScore = Math.round(z * 0.9 + (isMajorSetup ? 0.08 : 0.05));
    total += holeScore;
  }
  return total;
}
export default function Home() {
  ...
  const teamScores = Object.entries(TEAMS).map(([name, golfers]) => {
    const scores = golfers.map(g => {
      const p = players[normalizeName(g)];
  ...
  return (
    <>
      <Head>
        <title>Open Championship 2026 — Pool Leaderboard</title>
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Sans:wght@400;500;600&display=swap" rel="stylesheet" />
      </Head>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        :root{--bg:#0a0c0e;--surface:#111418;--surface2:#1a1f26;--border:#2a3040;--gold:#c9a24b;--text:#e8eaed;--muted:#8892a0;--green:#22c55e;--red:#ef4444;--r:10px}
        body{background:var(--bg);color:var(--text);font-family:'DM Sans',sans-serif;min-height:100vh}
        .hdr{background:linear-gradient(180deg,#120e08 0%,var(--bg) 100%);border-bottom:1px solid var(--border);padding:16px;text-align:center;position:sticky;top:0;z-index:50}
        ...
        .tab.active{background:var(--gold);border-color:var(--gold);color:#1a1204}
      ...
      `}</style>
      <div className="hdr">
        <div className="hdr-eye">Royal Birkdale · Southport, England</div>
        <div className="hdr-title">Open Championship 2026</div>
